...
/**
 * Creates the HTML for the search results page.
 * @param {string} query - The decoded search query.
 * @param {Array<Object>} results - The array of search result articles.
 * @returns {string} The complete HTML for the search results page.
 */
export function createSearchResultsView(query, results) {
    ...
    const title = hasResults
        ...
        : `No results for '<span class="query">${query}</span>'`;
    
    const subtitle = hasResults ? '' : 'Try a different search term.';
